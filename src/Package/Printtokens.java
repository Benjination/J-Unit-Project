...
public class Printtokens {
	...
	static int token_type(String tok) {
		if (is_keyword(tok))
			return (keyword);
		// Moved after is_str_constant as to not catch string constants
	...
	static boolean is_str_constant(String str) {
		...
		// { while (i < str.length() && str.charAt(i)!='\0')
		// Check there are not quotes in the middle of the string
		{
	...
	public static void main(String[] args) {
		...
		if (fname == null) {
			// Inform user and use System.in for input
			System.out.println("Please type tokens to classify (press Ctrl+C to end):");
		}

		BufferedReader br = t.open_token_stream(fname); // Open file
